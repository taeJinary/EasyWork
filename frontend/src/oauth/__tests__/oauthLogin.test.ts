...
describe('oauthLogin', () => {
  ...
  it('builds google authorize url with configured client id and callback uri', () => {
    ...
    expect(url.searchParams.get('redirect_uri')).toBe(`${window.location.origin}/oauth/google/callback`);
  ...
  it('stores state and redirects when starting naver oauth login', () => {
    ...
    expect(redirectedUrl.searchParams.get('redirect_uri')).toBe(`${window.location.origin}/oauth/naver/callback`);
